 ...
 */
int
Encoding::print_error(const char *format, ...)
{
    int r;
    va_list l;
 ...
 */
int
Encoding::append_buffer(buffer *buf, const void *data, unsigned int data_size)
{
    if (buf->bit)
    {
        buf->curr++;
 ...
 */
void
Encoding::append_buffer_bit(buffer *buf, int bit)
{
 ...
 */
void
Encoding::free_buffer(buffer *buf)
{
    free(buf->head);
    free(buf);
}
 ...
 **/
int
Encoding::uchar2utf8_size(const UTF32Char *ustr, int ustr_len)
{
    int size = 0;
 ...
 */
char *
Encoding::utf32toutf8(const UTF32Char *ustr, int ustr_len, char *str,
            int *str_size)
{
    int sbuf_size;
 ...
 */
int
Encoding::utf8toutf32(const char *str, int str_size, UTF32Char **ustr,
            int *ustr_len)
{
    int ulen;
    ...
    if (!ustr) { return 0; }
    if ((*ustr = static_cast<UTF32Char*>(malloc(sizeof(UTF32Char) * ulen))))
    {
        UTF32Char *u;
        const char *str_end;
...
print_time_diff(void)
{
    ...
    if (pre_time)
    {
        ...
        Encoding::print_error("[time] %s (diff %10.6lf)", datetime_buf, time_diff);
    }
    else
    {
        Encoding::print_error("[time] %s", datetime_buf);
    }
